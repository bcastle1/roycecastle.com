...
check("Deploy keeps the data deny rule continuously installed", deployScript.includes('DEPLOY_ITEMS=("${SOURCE_ITEMS[@]}" api)') && deployScript.includes('cp -a -- "$TARGET/data/.htaccess" "$BACKUP/data/.htaccess"') && deployScript.includes('mv -f -- "$STAGE/data/.htaccess" "$TARGET/data/.htaccess"') && deployScript.includes('mv -f -- "$BACKUP/data/.htaccess" "$TARGET/data/.htaccess"') && deployScript.indexOf('mv -f -- "$BACKUP/data/.htaccess" "$TARGET/data/.htaccess"') < deployScript.indexOf('for ((index=${#INSTALLED_ITEMS[@]} - 1'));
